...
{
	[AddComponentMenu("MultiLanguage/Text With Symbols", 10)]
	[ExecuteInEditMode]
	// Needed for culling images that are not used //
	public class MultiLanguageTextPic : MultiLanguageText, IPointerClickHandler, IPointerExitHandler, IPointerEnterHandler,
		ISelectHandler
	{
		/// <summary>
		/// Image Pool
		...
		[SerializeField] [Tooltip("Allow click events to be received by parents, (default) blocks")]
		...
		*/
		...
		private string red, blue, purple, yellow, orange, green, brown;
		...
		{
			base.Awake();
			MultiLanguageManagerOnOnLanguageChange();
			MultiLanguageManager.OnLanguageChange += MultiLanguageManagerOnOnLanguageChange;	
		}
							...
							foreach (IconName icon in inspectorIconList)
							{
								if (icon.name == spriteName)
								{
									...
									img.rectTransform.sizeDelta =
										new Vector2(fontSize * ImageScalingFactor * icon.scale.x,
											fontSize * ImageScalingFactor * icon.scale.y);
		...
		protected override void OnPopulateMesh(VertexHelper toFill)
		{
			...
			for (var i = 0; i < m_ImagesVertexIndex.Count; i++)
			{
				...
				if (endIndex < toFill.currentVertCount)
				{
					...
					positions.Add(new Vector2((vert.position.x + fontSize / 2), (vert.position.y + fontSize / 2)) +
					              imageOffset);
		...
		protected string GetOutputText()
		{
			...
			var indexText = 0;
			fixedString = text
				.Replace("${Red}", "<color=#F65937>" + red + "</color>")
				.Replace("${Blue}", "<color=#51A2FF>" + blue + "</color>")
				.Replace("${Purple}", "<color=#623FC9>" + purple + "</color>")
				.Replace("${Yellow}", "<color=#FFD627>" + yellow + "</color>")
				.Replace("${Orange}", "<color=#D94C08>" + orange + "</color>")
				.Replace("${Brown}", "<color=#58220C>" + brown + "</color>")
				.Replace("${Green}", "<color=#20D98E>" + green + "</color>");
			...
			foreach (Match match in s_HrefRegex.Matches(fixedString))
			{
				...
				else
				{
					...
					{
						m_HrefInfos[count].startIndex =
							s_TextBuilder.Length * 4; // Hyperlinks in text starting vertex indices;
